...
@st.cache_resource
def get_db():
    # Get the JSON as a string from streamlit secrets
    firebase_config = st.secrets["firebase"]
    
    # Convert to JSON string
    firebase_json = json.dumps(firebase_config)
    
    # Create a client from the JSON string
    db = firestore.Client.from_service_account_info(json.loads(firebase_json))
    return db
